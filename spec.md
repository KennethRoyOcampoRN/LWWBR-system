...
- **Rate limiting on `/auth/login`** (per IP and per account — two independent counters, not one shared one, so failures spread across different accounts on a resort's one shared front-desk IP don't rate-limit staff who never mistyped anything themselves; client decision, 2026-08-22) and progressive lockout after repeated failures: 10 failed logins in an hour locks the account, for 5 minutes on the first lockout and 10 minutes on every lockout after that — capped at 10 minutes, not continuing to escalate to hours or a full day (client decision, 2026-08-22, replacing the original M1 build's 30min → 2h → 24h scale).
