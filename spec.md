...
**Build for Vercel + Supabase. This is settled, not a menu.** The owner and system admin must be able to monitor the resort from anywhere including overseas, which makes a hosted deployment the requirement rather than an option.
**Development uses a hosted Supabase project — no Docker, no local CLI stack.** Development against a real hosted Supabase project (Postgres + Realtime + Storage + Auth) starts immediately, ahead of the Vercel side of the deployment. This still isn't a menu — Vercel deployment (even a preview) waits until M2 (§11), so the client sees a working local build before any Vercel spend, but the Supabase side is real from day one rather than emulated. This matters: it means the Supabase Realtime and Supabase Storage adapter implementations are **exercised for real from M0**, against the same APIs production will use, with no "works locally, breaks in production" swap waiting at the end.
- Connection strings and keys (`DATABASE_URL`, `DIRECT_URL`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`) are supplied by the client and read from environment variables only. **Never commit them** — `.env` stays gitignored; only `.env.example` with placeholder values is tracked.
- The API and web app run locally (`npm run dev`) against this hosted project. There is no separate local database to keep in sync — the hosted project *is* the dev database until M7 promotes to a production project (or the same project is hardened in place; decide at M7).
- Prisma migrations (`prisma migrate dev` / `prisma db push`) apply directly to the hosted project's direct connection (`DIRECT_URL`).
**First Vercel deployment (M2) — a preview deploy, not launch.** SPA and API both deploy to Vercel; the Express app is exported as a serverless handler. This first deploy exists to prove the serverless-safety rules below actually hold under Vercel's runtime, ahead of building four more milestones on top of an assumption that turned out wrong. Nobody at the resort uses this deploy for real operations — that's still gated on client sign-off at M6.5 and the full cutover at M7 (§11.1).
...
- **Prisma on serverless needs pooling.** Use Supabase's pooler on port `6543` with `?pgbouncer=true&connection_limit=1` as `DATABASE_URL`, and the direct `5432` connection as `DIRECT_URL` for migrations only. Skip this and you will exhaust connections within a day of real use. **Verify this at the M2 preview deploy** — hit an endpoint 50+ times in sequence and confirm no connection exhaustion — rather than discovering it for the first time at M7.
- **Pin the region.** Put the Supabase project and the Vercel function region both in `ap-southeast-1` (Singapore) — nearest to Batangas. Do not let the API float to a US default region while the database sits in Asia; every request would then cross the Pacific twice. An overseas viewer's latency is dominated by their own distance and is fine either way; the on-site staff are the ones who feel a misplaced region all day.
- Backups: Supabase automated daily backups on Pro, plus a scheduled `pg_dump` to separate off-site storage. Do not rely on a single vendor's backups for the property's financial records. Not needed until real data exists — set up at M7.
- **Plans are not optional once real operations depend on this.** Vercel Hobby forbids commercial use, and Supabase Free auto-pauses a project after 7 days of inactivity — fatal for a system that must be reachable at 2am on a quiet Tuesday. The M2 preview deploy can run on free tiers since it's a build-verification step, not a production dependency. Budget Vercel Pro (~$20/mo, one developer seat; resort staff are app users and do not need Vercel seats) and Supabase Pro (~$25/mo) before M7. Turn on spend caps on both no later than that.
...
**Write serverless-safe code from day one, even though local dev is a long-lived process.** This is the main trap in building locally and deploying to Vercel later, and it is invisible until launch day:
...
- `ISSUED` past `dueBackAt` auto-flips to `OVERDUE` via `POST /api/v1/jobs/amenity-overdue`, called every 15 minutes by Vercel Cron in production and triggered manually in local dev. Not a `setInterval` — see §3.1.
...
- **M0 — Scaffold.** Monorepo, Prisma schema, `RealtimeAdapter` and `StorageAdapter` interfaces with **Supabase implementations only**, dev running against a hosted Supabase project (no Docker, no local CLI stack — see §3.1), ESLint/Prettier, health check, test runner wired up. Vercel config is written to spec (function region `ap-southeast-1`, pooled + direct Prisma URLs, cron entries) but **not deployed** — the first deploy is M2. Acceptance: `npm run dev` boots web + api against the hosted Supabase project; `GET /api/v1/health` returns 200 and reports adapter resolution; a file uploads and reads back through `StorageAdapter` against the real Supabase Storage bucket; a realtime event round-trips through `RealtimeAdapter` against real Supabase Realtime; `npm test` runs green; a grep for `setInterval`, `setTimeout(`-based scheduling, and module-level mutable caches comes back clean.
...
- **M2 — Units & Command Center, first Vercel preview deploy.** Unit model, status state machine, unit grid, unit detail drawer, timeline, realtime status updates, plus the first Vercel deployment (§3.1 — a preview deploy to verify the serverless assumptions, not a launch). Acceptance: two browsers open the grid; a status change in one appears in the other within 2s without refresh; an invalid transition is rejected with a readable message; a stale `version` returns `409`; the Vercel preview deploy reaches Supabase through the pooler and survives 50 sequential requests without connection exhaustion.
...
1. **Create the Supabase project in `ap-southeast-1` (Singapore).** Region is fixed at creation; changing it later means migrating the database. Supabase Pro (~$25/mo) — the free tier auto-pauses after 7 days idle and will take the resort offline on a quiet week.
2. **Create the Vercel project, Pro plan** (~$20/mo, one developer seat — resort staff are app users and need no Vercel seats). Hobby forbids commercial use. Pin the function region to `ap-southeast-1` to match Supabase.
3. **Set both spend caps low** before the first deploy.
4. **Wire the pooled and direct Prisma URLs** (`6543` with `?pgbouncer=true&connection_limit=1` for `DATABASE_URL`; `5432` for `DIRECT_URL`), then run migrations.
5. **Re-verify connection pooling under load against the production project/deploy** — hit an endpoint 50+ times in sequence and confirm no connection exhaustion. This was already proven out once at the M2 preview deploy (§3.1); re-run it here because production is a different Vercel project (and possibly a different, hardened Supabase project) with its own pooler config, and that config is exactly what a copy-paste mistake breaks silently. Do this before staff are on the system, not after.
6. **Register the Vercel Cron entries** for the amenity-overdue sweep and the owner digest, and confirm each fires once against the deployed endpoints.
...
| 3 | **Cloud only: Vercel + Supabase.** The driving reason is that the owner and system admin must monitor the resort from off-site and from overseas, which an on-prem box cannot serve well. Adapter interfaces stay; on-prem implementations are not built. | §1, §3.0, §3.1, §3.1.1, M0 |
