...
- **Rate limiting on `/auth/login`** — a fast check, 5 failures / 15 minutes, evaluated as **two independent counters**, one scoped to the account, one to the IP (client decision, 2026-08-22, replacing an earlier single combined counter that let one account's failures inflate the count checked for a *different* account). The IP counter still legitimately blocks every account sharing a "hot" IP once the IP itself accumulates 5 failures against *any* account — that is the actual purpose of per-IP throttling (catching failures spread across many accounts from one source, e.g. a resort's shared front-desk connection under a credential-stuffing attempt) and was not changed. What the independent-counters fix removed was a clean account inheriting a rate-limit purely because of the *query shape*, not because its own IP was actually implicated.
- **Progressive lockout** after repeated failures, tracked separately from the rate limiter above: 10 failed logins for one account within an hour locks that account — 5 minutes on the first lockout, 10 minutes on the second and every one after, capped there rather than continuing to escalate to hours or a full day (client decision, 2026-08-22, replacing the original M1 build's 30min → 2h → 24h scale). Because the fast rate limiter's own threshold (5) is lower than the lockout's (10), it intercepts repeated rapid attempts first — reaching the 10-failure lockout requires failures spread across more than one 15-minute window, not a single burst of clicking.
- **Two-factor authentication (TOTP) required for `SYSTEM_ADMIN`**, optional for everyone else. This is the one account that can change permissions and reconfigure the system — a materially higher blast radius than any other role, including `OWNER`, which is read-only everywhere except `payment:verify`/`report:export` (§5.4) and so is deliberately not required to enroll (client decision, 2026-08-22).
...
- **M1 — Auth, RBAC & hardening.** User/Role/Permission models, login, refresh, `requirePermission` middleware, seed script, admin UI for users & roles, audit log middleware, plus the §3.1.1 security set: login rate limiting and lockout, TOTP 2FA enforced for `SYSTEM_ADMIN`, session list with remote revocation, HTTPS/HSTS/secure cookies. Acceptance: login as each seeded role and see a correctly filtered nav; a `403` is returned when calling an endpoint outside your permission set; every mutation appears in `AuditLog`; a system admin account cannot complete login without a TOTP code; revoking a session from another device invalidates its refresh token immediately; 10 failed logins lock the account and the attempts are visible in the audit log.
...
6. **Configure and confirm the Netlify Scheduled Functions** for the amenity-overdue sweep and the owner digest — this is a real production dependency to set up, not just a behavior to verify. `JOB_SECRET` must be rotated off the local-dev placeholder value to a real production secret in Netlify's environment config, and each Scheduled Function's own outbound call must be wired to send that exact value in the `x-job-secret` header — the endpoints reject anything else with `401`. Without this, `POST /api/v1/jobs/amenity-overdue` (built M5) never fires on its own in production: nothing else flips an `ISSUED` amenity request to `OVERDUE`. Confirm both are registered and each fires once against the deployed endpoints with a `200`, not a `401`.
7. **Confirm no public storage buckets** — payment proofs and waivers must 401 when fetched without a session.
8. **Force HTTPS/HSTS, secure cookies**, and confirm 2FA enrolment works for the System Admin account from a non-PHT network if possible.
...
All six original open questions have been answered by the client. These are settled — build to them, do not re-litigate. (Decision 7 below is a later addition, not one of the original six — see its own row for how it came about.)
...
| 7 | **Payment, Folio, and Cash-count tracking are unbuilt and explicitly out of scope for now.** This was never actually decided or written down at the time — it fell out of the 2026-08-24 "monitoring, not transactions" pivot (every reservation now arrives pre-booked through the external website, so Check-in never collects a rate — `BookingUnit.rate` is nullable specifically because of this) without anyone separately deciding that pivot should also remove payment recording, folio charges, and cash counts. It was noticed and made explicit on 2026-08-26, while building the Owner daily digest, when the digest's spec'd content (revenue, payment verification queue) had no data anywhere to draw from. Documented here, plainly, rather than left to be inferred from a nullable column and a report's quietly-omitted field. | §6 `Payment`/`FolioCharge`/`FolioSettlement`/`CashCount` (schema defined, zero code reads or writes them), M4 (built without this), §8.3 Owner dashboard/digest |
| 8 | **No owner daily digest email — ever, unless revisited.** The owner will monitor everything live through the Command Center / Owner dashboard instead of a scheduled 8:00 AM PHT email summary. This reverses §8.3's "Send a summary at 8:00 AM PHT" line. Reason given: live monitoring already covers what the digest existed to summarize, so a separate scheduled email is redundant rather than additive. The Owner dashboard is now the sole channel for this information — not "email, deprioritized," not "email later" — no digest send of any kind unless this decision is itself revisited. | §8.3 Owner dashboard ("Daily digest" bullet), M6 |
| 9 | **The Shift roster UI is removed from `/shifts`, kept only as dormant backend.** Staff scheduling is handled manually outside the app — staff already know their shifts without the system tracking it — so the "Add a shift" form, roster list, and Employee/Department/Date/Start-End-time/Reliever fields no longer render. Reason given: the roster added no value nobody was already getting from manual scheduling. DTR (Time Clock, clock-in/out), Rest-day requests, Flagged Entries, and Geofence settings are unaffected and continue to render exactly as before. The `Shift` Prisma model, its `shift:manage`/`shift:read` permission grants, and the backend `shifts` module's routes/service/tests all stay in the codebase untouched and dormant — not deleted — in case this is revisited; `shift:manage` continues to gate Flagged Entries review, independent of the roster UI it used to also gate. | §11 Phase-2 backlog ("shift roster + reliever assignment + DTR"), `/shifts` page built outside the original module list |
...
- **Decision 7 leaves decisions 4–6 unbuilt as written, plainly stated:**
  - **Cashier's whole spec'd role (§8.3) is currently unimplemented.** No new-booking form, no payment entry with proof upload, no open-folios list, no shift cash count. `CASHIER` today has no surface of its own to speak of.
  - **M4's stated acceptance criteria does not hold.** §11's M4 line describes "a full booking → payment → verify → check-in → charge → settle → check-out → auto-dirty cycle" where "check-out is refused with a non-zero folio balance" and "a manager can force it with a reason." What was actually built instead (the 2026-08-24 redesign) is a simpler check-in/out flow, monitoring a guest who already booked and paid externally, with no folio, no payment gate, and no forced-checkout override.
  - **The Owner dashboard and daily digest will never show "revenue in actual pesos," a payment verification queue, folio receivables outstanding, or a cash-variance exception alert until this is revisited.** Those fields are explicitly labelled as not tracked (§8.3) rather than estimated, faked, or silently dropped.
  - None of this is a recommendation to reopen Payment/Folio/CashCount — it's a record of what "monitoring, not transactions" actually excluded, so the gap is visible in the spec itself rather than only inferable from a nullable column and a report's omitted field.
- **Decision 8 makes the Owner dashboard the sole channel for what the digest used to summarize.** No 8:00 AM PHT email, no separate scheduled send, in any form — an owner who wants "yesterday at a glance" opens the dashboard rather than reading an inbox. The digest's own code (`sendOwnerDigest`, `computeDigestContent`, `renderDigestEmail` — spec §8.3, built M6) stays in the codebase, tested and working, but is deliberately not wired to any Netlify Scheduled Function, so it never actually fires in production. If this is revisited later, re-adding the schedule is a `netlify.toml` change, not a rebuild.
- **Decision 9 leaves the `shifts` module fully built but with no UI caller.** `Shift`'s schema, `shift:manage`/`shift:read` role grants, and `apps/api/src/modules/shifts/{schema,service,router}.ts` (with their own tests) are untouched and still mounted in `app.ts` — only `ShiftRosterSection` and its "Add a shift" form were deleted from `apps/web/src/routes/ShiftsPage.tsx`. `GET/POST/DELETE /shifts` and `GET /shifts/assignable-users` still work if called directly; nothing currently calls them. Re-adding the roster UI later is restoring one deleted component and its two lines of JSX, not a backend rebuild. `shift:manage` keeps its second, unrelated job of gating Flagged Entries review on the DTR side — that gate was never roster-specific and is unaffected.
