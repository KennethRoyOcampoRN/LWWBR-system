...
### 3.1 Deployment target — cloud, decided
...
- `ISSUED` past `dueBackAt` auto-flips to `OVERDUE` via a scheduled job every 15 minutes.
...
- **M0 — Scaffold.** Monorepo, Prisma schema, `RealtimeAdapter` and `StorageAdapter` interfaces with **Supabase implementations only**, Vercel config with the function region pinned to `ap-southeast-1`, pooled + direct Prisma URLs, ESLint/Prettier, health check, test runner wired up. Acceptance: `npm run dev` boots web + api against a local Postgres; a preview deploy to Vercel reaches Supabase through the pooler and survives 50 sequential requests without connection exhaustion; `GET /api/v1/health` returns 200 and reports region and adapter resolution; `npm test` runs green.
