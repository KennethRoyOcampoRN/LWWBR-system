...
**However: do not create cloud accounts yet.** The whole MVP is built and tested on a local stack first, so the client can use it, change his mind, and reshape it before anyone pays for hosting. Cloud signup happens at M7 (§11), not M0.
**Local development stack — use the Supabase CLI, not a bare Postgres container.**
`supabase start` runs Postgres, Realtime, Storage, Auth, and Studio in Docker on the developer's machine, free and with no account required. This matters: it means the Supabase Realtime and Supabase Storage adapter implementations are **exercised for real from M0**, against the same APIs production will use. A bare `postgres:16` container would force either a throwaway Socket.IO implementation or stubbed realtime, and the swap at launch is exactly the kind of "works locally, breaks in production" that this spec exists to avoid.
- `DATABASE_URL` and `DIRECT_URL` both point at the local instance during development.
- Realtime and Storage resolve to the local Supabase services.
- Going live is then a `.env` change plus `supabase db push`, not a re-architecture.
**Production target (M7):**
- SPA and API both deploy to Vercel; the Express app is exported as a serverless handler.
...
- **M0 — Scaffold.** Monorepo, Prisma schema, `RealtimeAdapter` and `StorageAdapter` interfaces with **Supabase implementations only**, local Supabase CLI stack, ESLint/Prettier, health check, test runner wired up. Vercel config is written to spec (function region `ap-southeast-1`, pooled + direct Prisma URLs, cron entries) but **not deployed** — no cloud accounts exist yet. Acceptance: `supabase start` and `npm run dev` bring up the full stack locally; `GET /api/v1/health` returns 200 and reports adapter resolution; a file uploads and reads back through `StorageAdapter` against local Supabase Storage; a realtime event round-trips through `RealtimeAdapter` against local Supabase Realtime; `npm test` runs green; a grep for `setInterval`, `setTimeout(`-based scheduling, and module-level mutable caches comes back clean.
